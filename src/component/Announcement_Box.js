  ...
  return (
    <div className="w-full border-2 rounded-md text-base border-gray-300  mb-4">
      ...
      <p className="p-4 py-2">
        {typeof data.data === "string" ? data.data &&parentId : <a onClick={() => {navigate(`/work/${parentId}/${id}/`)}} className="text-blue-600 duration-200 cursor-pointer text-base font-bold">{data.title}</a>}
      </p>
    </div>
  );
};
